...
class ParkingLot:
    ...
    def get_ped(self, time_slot, day_type, weather_type):
        ...
        # Time modifier
        if 0 <= time_slot < 13:  # Morning peak
            time_modifier = 0.7
        elif 13 <= time_slot < 17:  # Evening peak
            time_modifier = 1.27
        else:
            time_modifier = 1.0
        ped *= time_modifier
...
class QLearningAgent:
    def __init__(self):
        ...
        self.TOTAL_EPISODES = 10000  # Reduced for faster testing
        self.TIME_SLOTS = 24    
...
# Define the four scenarios: (day_type, weather_type)
scenarios = {
    "Weekday_Good": (0, 0),
    "Weekday_Bad": (0, 1),
    "Weekend_Good": (1, 0),
...
# Simulate each scenario and save separate plots
for name, (day_type, weather_type) in scenarios.items():
    time_slots, occupancies, prices, revenues, total_revenue = test_policy(
        trained_agent, parking_env, day_type=day_type, weather_type=weather_type, initial_occupancy=0.1
    )

    # Occupancy plot
    plt.figure(figsize=(8,5))
    plt.plot(time_slots, occupancies, marker='o')
    plt.xlabel("Time Slot (Hour)")
    plt.ylabel("Occupancy (Fraction of Capacity)")
    plt.title(f"Occupancy - {name.replace('_', ' ')}")
    plt.tight_layout()
    plt.savefig(f"occupancy_{name}.jpg", dpi=300)
    plt.close()

    # Price plot
    plt.figure(figsize=(8,5))
    plt.plot(time_slots, prices, marker='o', color='orange')
    plt.xlabel("Time Slot (Hour)")
    plt.ylabel("Dynamic Price")
    plt.title(f"Price - {name.replace('_', ' ')}")
    plt.tight_layout()
    plt.savefig(f"price_{name}.jpg", dpi=300)
    plt.close()

    # Revenue plot
    plt.figure(figsize=(8,5))
    plt.plot(time_slots, revenues, marker='o', color='green')
    plt.xlabel("Time Slot (Hour)")
    plt.ylabel("Revenue")
    plt.title(f"Revenue - {name.replace('_', ' ')}")
    plt.tight_layout()
    plt.savefig(f"revenue_{name}.jpg", dpi=300)
    plt.close()

    # Print scenario stats
    avg_occ = np.mean(occupancies)
    avg_price = np.mean(prices)
    print(f"{name.replace('_', ' ')} | Avg Occupancy: {avg_occ:.2f} | Avg Price: {avg_price:.2f} | Total Revenue: {total_revenue:.2f}")
